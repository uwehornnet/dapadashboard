...
const TableRow = ({ index, id, data, status, invoice, location }) => {
	const navigate = useNavigate();
	const { kennzeichen, hersteller, typ, kraftstoff } = data ? JSON.parse(data) : {};
	return (
		<tr className={index % 2 === 0 ? null : "bg-gray-50"}>
			...
			<td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex justify-end items-center space-x-4 text-slate-500">
				{invoice === null ? (
					...
					<a
						href={`${baseURI}/api/invoice/pdf/${invoice}`}
						target="_blank"
						className={`cursor-pointer hover:text-slate-900`}
					>
						<svg
				...
				{loading ? (
					...
					<div>
						{data.data.length ? (
							<>
								<Table
									head={["ID", "Kennzeichen", "Hersteller/Typ", "Kraftstoff", "Standort", "Status"]}
								>
